...
-- ============================================
-- HARDENED RLS PATCH: complete hotel scoped policies
-- Added by production-readiness patch. Keep idempotent for re-runs.
-- ============================================

DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'user_profiles' AND policyname = 'profiles_same_org_select') THEN
...
BEGIN
  FOREACH table_name IN ARRAY ARRAY['rooms','room_types','folios','invoices','payments','housekeeping_tasks'] LOOP
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = table_name AND policyname = 'hotel_data_isolation_all') THEN
      EXECUTE format('CREATE POLICY "hotel_data_isolation_all" ON %I FOR ALL USING (hotel_id IN (SELECT id FROM hotels WHERE organization_id = auth.user_organization_id())) WITH CHECK (hotel_id IN (SELECT id FROM hotels WHERE organization_id = auth.user_organization_id()))', table_name);
    END IF;
  END LOOP;
END $$;
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'folio_items' AND policyname = 'folio_items_via_folio_all') THEN
    CREATE POLICY "folio_items_via_folio_all" ON folio_items FOR ALL
      USING (folio_id IN (
        SELECT id FROM folios WHERE hotel_id IN (
          SELECT id FROM hotels WHERE organization_id = auth.user_organization_id()
        )
      ))
      WITH CHECK (folio_id IN (
        SELECT id FROM folios WHERE hotel_id IN (
          SELECT id FROM hotels WHERE organization_id = auth.user_organization_id()
        )
      ));
